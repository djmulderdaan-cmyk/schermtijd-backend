...
async function init() {
  await pool.query(`
    ...
    CREATE TABLE IF NOT EXISTS rules (
      ...
      locked_date TEXT,
      -- SHA-256-hash (nooit de platte PIN) van de noodcode. In de database i.p.v. alleen in het
      -- geheugen van de server: gratis hosting (bv. Render) herstart het serverproces geregeld
      -- (spin-down bij inactiviteit), waardoor een puur in-memory opslag na elke herstart leeg was
      -- en een nieuw gekoppeld kind-apparaat dan geen noodcode kreeg, ook al had de ouder er allang
      -- een ingesteld. Blijft privacyvriendelijk: dit is en blijft alleen de hash, nooit de PIN zelf.
  ...
  await pool.query(`ALTER TABLE rules ADD COLUMN IF NOT EXISTS emergency_code_hash TEXT;`);
}
