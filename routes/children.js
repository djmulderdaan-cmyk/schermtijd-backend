...
// de platte-tekst-PIN komt nooit op de server of in de database terecht, alleen de hash. Die hash
// staat in rules.emergency_code_hash (i.p.v. alleen in het geheugen van de server) zodat hij een
// serverherstart overleeft — belangrijk op gratis hosting die het proces regelmatig herstart.
router.put('/:id/emergency-code', async (req, res) => {
  try {
    ...
    await pool.query('UPDATE rules SET emergency_code_hash = $1 WHERE child_id = $2', [codeHash, childId]);
    res.json({ ok: true });
  } catch (e) {
    console.error(e);
