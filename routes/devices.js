...
// zelf). Geeft null terug als er nog geen noodcode is ingesteld. Staat in rules.emergency_code_hash
// zodat dit een serverherstart overleeft (zie routes/children.js PUT /:id/emergency-code).
router.get('/:id/emergency-code', async (req, res) => {
  try {
    if (req.device.id !== Number(req.params.id)) return res.status(403).json({ error: 'Niet jouw apparaat' });
    const result = await pool.query('SELECT emergency_code_hash AS "codeHash" FROM rules WHERE child_id = $1', [req.device.childId]);
    res.json({ codeHash: result.rows[0]?.codeHash || null });
  } catch (e) {
    console.error(e);
    res.status(500).json({ error: 'Databasefout bij ophalen noodcode' });
  }
});
