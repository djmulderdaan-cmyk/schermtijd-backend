...
// E-mailadres van een account wijzigen (bv. typfout bij registratie, of account overdragen).
router.put('/accounts/:id/email', async (req, res) => {
  try {
    const userId = Number(req.params.id);
    ...
    if (existing.rows.length > 0) return res.status(409).json({ error: 'Dit e-mailadres is al in gebruik door een ander account' });

    const result = await pool.query('UPDATE users SET email = $1 WHERE id = $2', [email, userId]);
    if (result.rowCount === 0) return res.status(404).json({ error: 'Account niet gevonden' });
    res.json({ ok: true, email });
  } catch (e) {
    console.error(e);
    res.status(500).json({ error: 'Databasefout bij wijzigen e-mailadres' });
  }
});
...
// Dagelijkse limiet en bedtijdvenster van een kind bewerken, ongeacht wie de eigenaar is —
// zelfde velden als het gewone ouder-dashboard, maar dan voor ondersteuningsdoeleinden.
router.put('/children/:childId/rule', async (req, res) => {
  try {
    ...
    console.error(e);
    res.status(500).json({ error: 'Databasefout bij bewerken regel' });
  }
});

// Verwijdert 1 kind (en alles eronder: apparaten, categorieën, koppelcodes, regel).
router.delete('/children/:childId', async (req, res) => {
  ...
  try {
    ...
    const devicesResult = await client.query('SELECT id FROM devices WHERE child_id = $1', [childId]);
    const deviceIds = devicesResult.rows.map((r) => r.id);
    if (deviceIds.length > 0) {
      await client.query('DELETE FROM usage_logs WHERE device_id = ANY($1)', [deviceIds]);
    }
    await client.query('DELETE FROM devices WHERE child_id = $1', [childId]);
    await client.query('DELETE FROM app_categories WHERE child_id = $1', [childId]);
    await client.query('DELETE FROM category_rules WHERE child_id = $1', [childId]);
    await client.query('DELETE FROM pairing_codes WHERE child_id = $1', [childId]);
    await client.query('DELETE FROM rules WHERE child_id = $1', [childId]);
    const deleteResult = await client.query('DELETE FROM children WHERE id = $1', [childId]);
    ...
    await client.query('ROLLBACK');
    console.error(e);
    res.status(500).json({ error: 'Databasefout bij verwijderen kind' });
  } finally {
    client.release();
  }
});

// Verwijdert 1 apparaat (ontkoppelen namens de ouder, bv. als een apparaat kwijt/vervangen is).
router.delete('/devices/:deviceId', async (req, res) => {
  try {
    const deviceId = Number(req.params.deviceId);
    await pool.query('DELETE FROM usage_logs WHERE device_id = $1', [deviceId]);
    ...
    console.error(e);
    res.status(500).json({ error: 'Databasefout bij verwijderen apparaat' });
  }
});
